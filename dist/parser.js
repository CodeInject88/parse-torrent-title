...
import { cleanTitle, beforeTitleRegex, whitespacesRegex, underscoresRegex, matchIndices, trailingEpisodePattern, extractEpisodeTitle, wordCharRegex } from './utils.js';
import { GramSet, gateAllows, openPrefilter, prefilterFor } from './prefilter.js';
 ...
 */
export function parse(title, handlers, usePrefilter = true) {
    ...
    for (let hi = 0; hi < handlers.length; hi++) {
        ...
        if (!(nonAscii && unicodeSensitive[hi] === 1)) {
            ...
            if (off >= 0 && !gateAllows(gates, off, bits))
                continue;
        }
        ...
        if (handler.pattern) {
            ...
            if (handler.retryPastTitle &&
                ...
                while ((next = g.exec(title)) !== null) {
                    if (next.index >= endOfTitle)
                        break;
                    if (g.lastIndex === next.index)
                        g.lastIndex++;
                }
                if (next === null) {
                    continue;
        ...
        const isFirstEpisodeMarker = field === 'episodes' && !episodeMarkerSeen && m.mValue !== '';
        let markerAnchorsTitle = false;
        if (isFirstEpisodeMarker) {
            const leadingBracket = title.charCodeAt(0) === OPEN_SQUARE_BRACKET
                ? beforeTitleRegex.exec(title)
                : null;
            // A marker whose text begins inside a word is a mis-parse
            // A digit before it is normal
            const startsMidWord = m.mIndex > 0 &&
                letterRegex.test(title[m.mIndex - 1]) &&
                wordCharRegex.test(m.mValue[0]);
            const insideLeadingBracket = leadingBracket !== null && m.mIndex < leadingBracket[0].length;
    ...
    for (const [field, fieldMeta] of result) {
        ...
        if (v instanceof ValueSet) {
            ...
            if (field === 'languages' && values.includes('es-419')) {
                ...
                finalResult[field] = values.filter((lang) => lang !== 'es');
            }
            else {
                finalResult[field] = values;
            }
        }
        else {
            finalResult[field] = v;
    ...
    if (finalResult.episodes &&
        ...
        const episodeTitle = extractEpisodeTitle(title, episodeTitleStart, finalResult.group);
        if (episodeTitle) {
