...
function foldChar(code) {
    if (code >= 65 && code <= 90)
        return code + 32; // ASCII upper -> lower
...
export class GramSet {
    ...
    addRange(title, from, to) {
        ...
        let prev1 = -1;
        if (start >= 2)
            prev2 = foldChar(title.charCodeAt(start - 2));
        if (start >= 1)
            prev1 = foldChar(title.charCodeAt(start - 1));
...
function literalGrams(literal) {
    ...
    for (let i = 0; i < literal.length; i++) {
        if (literal.charCodeAt(i) >= 128)
            return [BIT_NON_ASCII];
    }
    ...
    if (!shaped) {
        if (n === 1)
            return [BIT_CHAR_BASE + codes[0]];
        if (n === 2)
            return [biHash(codes[0], codes[1])];
    }
    else {
        for (let i = 0; i < n; i++)
            codes[i] = foldShape(codes[i]);
        // One folded character says nothing beyond "a digit is present".
        if (n === 1)
            return [];
        if (n === 2)
            return [shapeBiHash(codes[0], codes[1])];
    }
    const positions = n - 2;
...
function crossProduct(left, right) {
    ...
    if (leftNonAscii !== right.some(hasNonAscii))
        return null;
    const out = [];
    for (const a of left) {
...
class SourceParser {
    ...
    parseSequence() {
        const items = [];
        let open = false;
        const append = (set, keepOpen) => {
            const last = items[items.length - 1];
            const merged = open && last !== undefined && last.t === 'lit'
        ...
        while (this.i < this.src.length) {
            ...
            if (quant === null) {
                ...
                append([atom.set[0].repeat(copies)], quant.max === quant.min && quant.min <= MAX_REPEAT_EXPANSION);
            }
            else {
    ...
    parseQuantifier() {
        ...
        if (ch === '*') {
            ...
            if (!m)
                return null; // a literal "{...}", not a quantifier
            min = parseInt(m[1], 10);
            max =
        ...
        if (this.src[this.i] === '?')
            this.i++; // lazy marker, same bounds
        return { min, max };
    }
    parseAtom() {
        const src = this.src;
    ...
    parseCharClass() {
        ...
        if (size > MAX_DIGIT_CLASS_ENUMERATION &&
    ...
    readClassChar() {
        ...
        if (next === undefined || /[dDwWsSpPbB0-9]/.test(next))
            return null;
        if (next === 'x' || next === 'u') {
            const hex = next === 'x'
    ...
    parseEscape() {
        ...
        this.i += 2;
        if (ch in controls)
            return { t: 'lit', set: [controls[ch]], exact: true };
...
function literalStrength(literal) {
    let digits = 0;
    for (let i = 0; i < literal.length; i++) {
        ...
        case 'alt': {
            ...
            for (const option of node.options) {
                ...
                if (group === null)
                    return null;
                all.push(...group);
                if (all.length > MAX_ALTERNATIVES)
                    return null;
            }
...
export function deriveLiteralGroups(pattern) {
 ...
 */
function isUnicodeSensitive(pattern) {
    return pattern.flags.includes('u') && pattern.flags.includes('i');
}
export function buildPrefilter(handlers) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (!handler.pattern)
            continue;
        if (isUnicodeSensitive(handler.pattern))
            unicodeSensitive[i] = 1;
        // An alternative with no grams is vacuously satisfied, which makes its
        // whole group unable to reject anything.
        const groups = deriveLiteralGroups(handler.pattern).filter((group) => group.every((literal) => literalGrams(literal).length > 0));
        if (groups.length === 0)
            continue;
        ...
        if (inline.length === 0 && remaining.length === 0)
            continue;
        gatedCount++;
...
export function gateAllows(gates, offset, bits) {
    let p = offset;
...
export function openPrefilter(size) {
 ...
 */
export function prefilterFor(handlers) {
