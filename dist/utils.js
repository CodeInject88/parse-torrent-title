 ...
 */
export const extensions = '3g2|3gp|avi|flv|mkv|mk3d|mov|mp2|mp4|m4v|mpe|mpeg|mpg|mpv|webm|wmv|ogm|divx|ts|m2ts|iso|vob|sub|idx|ttxt|txt|smi|srt|ssa|ass|vtt|nfo|html';
...
export function cleanTitle(rawTitle) {
    ...
    if (title.indexOf('_') !== -1)
        title = title.replace(underscoreRegex, ' ');
...
const episodeTitleTrailingTagRegex = new RegExp('^(?:' + ['\\d{3,4}[pi]', '[48]k', 'u?hd', 'dl', 'synced'].join('|') + ')$', 'i');
...
function isStopWord(token) {
    ...
    return (episodeTitleHardStopRegex.test(word) ||
...
function isTrailingTag(token) {
    const word = token.replace(surroundingPunctuationRegex, '');
    return (episodeTitleTrailingTagRegex.test(word) ||
 ...
 */
export function extractEpisodeTitle(w, start, group) {
    ...
    let sepsBefore = 0;
    for (let i = start - 1; i >= 0 && isSep(w[i]); i--)
        sepsBefore++;
    ...
    if (Math.min(sepsBefore, 1) + sepsAfter !== 1 || i >= w.length)
        return null;
    const tokens = [];
    let terminator = '';
    while (i < w.length) {
    ...
    if (episodeTitleHardStopRegex.test(firstWord))
        return null;
    ...
    if (episodeTitleLanguageWordRegex.test(firstWord) &&
    ...
    if (/^(?:\d{3,4}[pi]|[48]k)$/i.test(firstWord))
        return null;
...
const extensionRegex = new RegExp(String.raw `(?:[. ][a-z]{2,3}(?:-[a-z]{2})?)?[. ](?:${extensions})$`, 'i');
...
const trailingGroupRegex = new RegExp(String.raw `(?:([\s.])-[\s.]|-)(${groupToken})$`);
const episodeLikeRegex = /^(?:\d+|\d+[a-z]|\d+v\d+|\d+x\d+|s\d+\w*|ep?\d+\w*)$/i;
...
function hasToken(text, token) {
    for (let i = text.indexOf(token); i !== -1; i = text.indexOf(token, i + 1)) {
        ...
        if ((i === 0 || !wordCharRegex.test(text[i - 1])) &&
...
export function extractGroup(w, result) {
    const claimed = (token) => {
        ...
        for (const [field, meta] of result) {
            ...
            for (let i = 0; i <= texts.length; i++) {
                ...
                if (i > last && last >= 0 && text === texts[last])
                    break;
    ...
    const leadingGroup = leading !== null && !/^[\s.\-_]|[\s.\-_]$|[\s.\-_]{2}/.test(leading[1])
        ? leading[1]
        : null;
    ...
    for (;;) {
        ...
        if (bm === null)
            break;
        const gm = afterTags && /[\s.]/.test(bm[1]) ? bracketGroupRegex.exec(s) : null;
 ...
 */
export function matchIndices(match, str) {
    const indices = new Array(match.length * 2);
