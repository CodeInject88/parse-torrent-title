...
import { toValue, toLowercase, toUppercase, toTrimmed, toCleanDate, toCleanMonth, toDate, toYear, toIntRange, toWithSuffix, toBoolean, toValueSet, toValueSetWithTransform, toValueSetMultiWithTransform, toIntArray, toIntRangeTill } from './transforms.js';
import { validateAnd, validateNotAtStart, validateNotAtEnd, validateNotMatch, validateNotStartSpaced, validateMatch, validateMatchedGroupsAreSame, validateLookbehind, validateOr, validateLookahead } from './validators.js';
...
export const handlers = [
    ...
    },
    {
        ...
        validateMatch: validateAnd(validateNotMatch(/(?:(?:seasons?|[Сс]езони?)\P{L}*)/iu), (input, idxs) => {
    ...
    },
    {
        ...
        validateMatch: validateAnd(validateNotMatch(/(?:seasons?|[Сс]езони?)\P{L}*|^(?:20-20)/iu), validateOr(validateLookbehind('Tatsuki[\\s._-]Fujimoto', 'i', false), validateNotMatch(/\b17-26\b/))),
        transform: toIntRange()
    },
    {
        field: 'episodes',
    ...
    },
    {
        ...
        transform: (title, m, result) => {
            ...
            const sm = result.get('seasons');
            const parsedSeason = sm && Array.isArray(sm.value) && sm.value.length === 1
                ? sm.value[0]
                : undefined;
    ...
    },
    {
        ...
        process: (title, m, result) => {
            ...
            if (match && match.index !== undefined) {
                ...
                const nextCharIsWordChar = matchEndPos >= beginningTitle.length &&
            ...
            if (!mStr && endIndex > 0 && endIndex < title.length) {
                ...
                if (dotMatch && dotMatch.index !== undefined && dotMatch[1]) {
                    ...
                    const sm = result.get('seasons');
                    const overlapsSeasons = sm !== undefined &&
                        sm.mValue !== '' &&
                        capStart >= sm.mIndex &&
                    ...
                    const yearInBetween = ym !== undefined && ym.mIndex >= capStart && ym.mIndex <= endIndex;
    ...
    },
    {
        field: 'languages',
        pattern: /\bde\b/i,
        validateMatch: validateLookahead('(?:[ .,/()\\[\\]-]+(?:[A-Z]{2}[ .,/()\\[\\]-]+){1,}[A-Z]{2}(?:[ .,/()\\[\\]-]+|$))', 'i', true),
        transform: toValueSet('de'),
        keepMatching: true,
        skipFromTitle: true
    },
    ...
    {
        field: 'subbed',
        pattern: /\b(?:DAN|E|FIN|PL|SLO|SWE|HEB|NOR|GER|ITA|SPA|POR|DUT|NL|CZE|GRE|TUR|ARA|RUS|HUN|HIN|ENG|KOR|JPN|CHI|VIE|THA)SUBS?\b/i,
        transform: toBoolean(),
        keepMatching: true,
        remove: true,
        skipIfFirst: true
